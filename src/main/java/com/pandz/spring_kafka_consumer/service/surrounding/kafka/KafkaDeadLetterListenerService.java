...
public class KafkaDeadLetterListenerService {
    @KafkaListener(topics = TOPIC_GENERAL + ".DLT", groupId = "${spring.kafka.consumer.group-id}")
    public void listenMessage(String message) {
        try {
            log.info("Incoming Message from DLT: {}", message);
        } catch (Exception e) {
    ...
    @KafkaListener(topics = TOPIC_CUSTOMER + ".DLT", groupId = "${spring.kafka.consumer.group-id}")
